...
def get_device():
    return "cuda" if torch.cuda.is_available() and not 'store_true' else "cpu"


def get_args(model_name):
    return torch.load(os.path.join(model_name, 'training_args.bin'))
                ...
                if len(words)<args.max_seq_len:
                  ...
                  for word in words:
                    ...
                    for i in range(len(word_tokens)):
                        try:
                          ner_embed.append(all_vocab[word.lower()]*(i+1))
                        except:
...
def upload_file():
    if request.method == 'POST':
        ...
        final_intent_pred, line = predict(sent,model_name)
        
        return render_template('result.html', class_id=line,
                               class_name=final_intent_pred)
